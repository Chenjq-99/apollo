...
Path::Path(const std::vector<LaneSegment>& segments)
    ...
    /*
        ����ͨ���е�ÿһ��LaneSegment����ȡ��һ��MapPathPoint
    */
...
/*
    �Ѵ�ͨ������ɢ�õ��ĵ�(MapPathPoint)���������һ��LaneSegment2d(��ʵ����LineSegment2d), 
    ��std::vector<common::math::LineSegment2d> segments_�洢

    Ϊʲô��������RoutSegment�е�LaneSegment�ж�Ӧ��lane�еĵ��ó������ֻ�ԭ��ȥ�ˣ�
    �£�1.�����е㶼�ó�����һ������(ȥ�������)
        2.�����൱�����ó������ⲿ�ֵ㣬������������һ��αlane(������������ɣ�ʹ��vector<common::math::LineSegment2d>)��
        �����s�Ǵ�0��ʼ�ģ��൱��������ԭ����lane�����ں�������
        3.��Щ����ܻ�������һ��lane�� ��ô���ѵ㶼�ó����ˣ�������һ���µ�ͳһ��αlane,���������������
*/
void Path::InitPoints() {
  ...
  for (int i = 0; i < num_points_; ++i) {
    ...
    if (i + 1 >= num_points_) {
      heading = path_points_[i] - path_points_[i - 1];
        ...
        /*
            ������ֱ����MapPathPoint����LineSegment2d������ΪMapPathPoint�̳���Vec2d
        */
  ...
  CHECK_EQ(segments_.size(), static_cast<size_t>(num_segments_));
    ...
    /*
        ���������һ��СLaneSegment2d������ͬһ������ƴ�ӳ�LaneSegment
        ��std::vector<LaneSegment> lane_segments_�洢
    */
...
/*
���õ���һ��LineSegment2d(std::vector<common::math::LineSegment2d> segments_),Ҳ����αlane
������������ɢ����ÿ��(kSampleDistance == 0.25m)����һ����
*/
void Path::InitPointIndex() {
  ...
  int last_index = 0;
  for (int i = 0; i < num_sample_points_; ++i) {
    // ���������õ�������i(��Ӧ�ۻ�����Ϊs)���½�MapPathPoint������last_index
    while (last_index + 1 < num_points_ &&
           accumulated_s_[last_index + 1] <= s) {
      ++last_index;
    }
    last_point_index_.push_back(last_index); // ��һ����������ۻ����룬����0.25
    s += kSampleDistance;
  }
...
//   ѭ��1
  for (const auto& lane_segment : lane_segments_) {
    ...
    // GetOverlaps_from_lane��֮ǰ��std::bind(&LaneInfo::cross_lanes, _1)���������LaneInfo���͵�lane�����thisָ��
...
bool Path::GetProjection(const Vec2d& point, double* accumulate_s,
  ...
  *min_distance = std::sqrt(*min_distance);
  const auto& nearest_seg = segments_[min_index];
  const auto prod = nearest_seg.ProductOntoUnit(point); // l方向投影
  const auto proj = nearest_seg.ProjectOntoUnit(point); // s方向投影
  if (min_index == 0) {
    *accumulate_s = std::min(proj, nearest_seg.length());
    if (proj < 0) {
      *lateral = prod;
